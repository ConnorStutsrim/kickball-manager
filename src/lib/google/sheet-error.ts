...
export function describeGoogleSheetsError(err: unknown): string {
  ...
  console.error("Unexpected error generating Google Sheet:", sanitizeForLog(err));
  return "Something went wrong generating the sheet. Check the server logs if this keeps happening.";
}

// Logging the raw thrown value is unsafe here — a Gaxios/Google API error's
// nested request config can carry the refresh token itself (confirmed by
// directly reproducing the invalid_grant failure this file was written
// for: the token appeared in the error's own config.data). A stack trace
// is generated by the JS engine at throw time from the call site alone, so
// it doesn't carry request/response payloads the way the full error object
// does — safe to log for debugging without that risk.
